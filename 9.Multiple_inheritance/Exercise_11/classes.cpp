...
std::ostream& SuperlativeAmoeba::printObject(std::ostream& os) const
{
	return os << "SuperlativeAmoeba has strengthFactor = " << getStrengthFactor() << std::endl;
}
...
SuperlativeAmoeba& SuperlativeAmoeba::operator=(const SuperlativeAmoeba& other)
{
	...
	if(this == &other)
		return *this;
	// Calling protected methods for assign to base parts: SuperlativeMan and Amoeba
	copyAssignSuperlativeManPart(other);
	copyAssignAmoebaPart(other);
	// Calling assignment operator for base class for SuperlativeMan and Amoeba - SuperHero
	// So, by using this approach we imitaite constructor behaviour when 
	// we have virtual inheritance and constructors of base classes initialize only their
	// parts and not initialize their base class - it make one time from inheritated class
	// of this classes. We reduce number of callings assignment operators of base base class
	SuperHero::operator=(other);
	return *this;
}
